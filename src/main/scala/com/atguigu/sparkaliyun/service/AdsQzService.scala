...
object AdsQzService {
    def getTarget(sparkSession: SparkSession, dt: String) = {
        val avgDetail = DwsQzDao.getAvgSPendTimeAndScore(sparkSession, dt)
        val topscore = DwsQzDao.getTopScore(sparkSession, dt)
        val top3UserDetail = DwsQzDao.getTop3UserDetail(sparkSession, dt)
        val low3UserDetail = DwsQzDao.getLow3UserDetail(sparkSession, dt)
        val paperScore = DwsQzDao.getPaperScoreSegmentUser(sparkSession, dt)
        val paperPassDetail = DwsQzDao.getPaperPassDetail(sparkSession, dt)
        val questionDetail = DwsQzDao.getQuestionDetail(sparkSession, dt)
    }
