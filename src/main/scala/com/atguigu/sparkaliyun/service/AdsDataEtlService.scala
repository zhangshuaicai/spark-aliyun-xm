...
import com.atguigu.sparkaliyun.bean.QueryResult
import com.atguigu.sparkaliyun.dao.DwsMemberDao
import org.apache.spark.sql.expressions.Window
import org.apache.spark.sql.{SaveMode, SparkSession}

object AdsDataEtlService {

    def queryDetailApi(spark: SparkSession, dt: String) = {
        import spark.implicits._
        val result =DwsMemberDao.queryIdlMemberData(spark).as[QueryResult].where(s"dt='${dt}'")
        result.cache()
        //统计注册来源url人数
    ...
    def queryDetailSql(spark: SparkSession, dt: String): Unit = {
        val appregurlCount = DwsMemberDao.queryAppregurlCount(spark, dt)
        val siteNameCount = DwsMemberDao.querySiteNameCount(spark, dt)
        val regsourceNameCount = DwsMemberDao.queryRegsourceNameCount(spark, dt)
        val adNameCount = DwsMemberDao.queryAdNameCount(spark, dt)
        val memberLevelCount = DwsMemberDao.queryMemberLevelCount(spark, dt)
        val vipLevelCount = DwsMemberDao.queryVipLevelCount(spark, dt).show()
